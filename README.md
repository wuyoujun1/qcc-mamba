...
> 🔧 **未解决的问题清单：[`docs/TOFIX.md`](docs/TOFIX.md)**
...
另外，**论文正文与代码有以下已知差异**。2026-09-12 决定：**以正文为准，代码暂不改**，写/改正文时知道就行（完整记录见 [`docs/TOFIX.md`](docs/TOFIX.md)）：
...
| 残差更新写 `H′ = LN(H + γ·H_p)` | gate 开时是 `H + γ·LN(H_p)`：**有内层 LN、没有外层 LN**，位置正好相反 |
...
> 其中第一条（两层都纠缠，n = 2）是 2026-09-12 明确固定的**理论口径**：正文（与 `qcc/feature_map.py` 文件头的数学定义）为准，代码实现只 1 层，**确定暂不改**。

**两个口径待确认**：
- `kernel_fn`：正文写保真度核 `K=|⟨ψ|ψ⟩|²`；早期架构文档曾说最终用 `quantum_exp`（测地核＋可学习 κ），主表逐格命令行已不可考。
