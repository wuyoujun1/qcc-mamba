...
E.append(Paragraph("图 3 给出一个定性案例。在 ETTh1 的 96 步预测上，油温 OT 的真实曲线、S-Mamba 与 QCCK-M 的预测如图所示：S-Mamba 的预测整体偏低并在上升段明显滞后，QCCK-M 则更好地跟随了真值的走势，前 60 步的 MSE 由 0.0293 降到 0.0143。这一案例与上文的耦合结构一致，耦合矩阵中读出的通道间强弱关系，在预测中体现为模型对不同通道的跟随能力。", body))
E += fig(os.path.join(FIGS, "ch5_pred_ETTh1.png"), 13*cm, "图 3　ETTh1 在 96 步预测下 OT 通道的定性案例（真值、S-Mamba 与 QCCK-M）。")
