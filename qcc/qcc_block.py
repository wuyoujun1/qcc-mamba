...
class QCCBlock(nn.Module):
    ...
    Args:
        ...
        angle_norm: 角度归一化 "clamp" / "sphere"（feature map 用）。
        angle_radius: 球面归一化半径（feature map 用，见 EntanglingFeatureMap）。
        corr_use_S: 修正项是否直接拼接对齐后频谱 S（"特征换位置"变体）。
        corr_gate_S: 修正项是否用频谱门控（逐变量 σ(Linear(S)) ⊙ corr，"门控"变体）。
        corr_use_ymain: 修正项是否拼接主预测 y_main（误差反馈变体）。
        corr_use_Hp: 修正项是否拼接独立 LN 的消息传递输出 Hp（"独立通道"变体，
            防止 Hp 的 S 调制被残差结构 H+Hp 淹没）。
        corr_mlp: 修正头是否用 MLP（Linear-GELU-Linear）替代单线性层
            （残差通常不是 backbone 特征的线性可读函数）。
    """
    ...
    def __init__(
        ...
        corr_use_S: bool = False,
        corr_gate_S: bool = False,
        corr_use_ymain: bool = False,
        corr_use_Hp: bool = False,
        corr_mlp: bool = False,
    ):
        super().__init__()
        ...
        if use_S:
            ...
            if corr_use_S or corr_gate_S:
                # S → 逐变量门控标量（sigmoid，0~1）
                self.s_gate = nn.Sequential(nn.Linear(2 * M, 16), nn.GELU(), nn.Linear(16, 1))
                nn.init.zeros_(self.s_gate[0].weight); nn.init.zeros_(self.s_gate[0].bias)
                nn.init.zeros_(self.s_gate[2].weight); nn.init.zeros_(self.s_gate[2].bias)

        self.corr_use_S = corr_use_S
        self.corr_gate_S = corr_gate_S
        self.corr_use_ymain = corr_use_ymain
        self.corr_use_Hp = corr_use_Hp
        self.corr_mlp = corr_mlp

        if corr_use_Hp:
            # Hp 独立通道：消息传递输出单独 LN（幅度仅 ~1.5% of H，直接拼接会被 H 淹没）
            self.hp_ln = nn.LayerNorm(d_token)

        # 跨变量特征 → 预测修正 (B, V, d [+2M] [+H] [+d]) → (B, V, H)
        corr_in_dim = (
            d_token
            + (2 * M if (corr_use_S and use_S) else 0)  # S 仅在 use_S 时可用
            + (horizon if corr_use_ymain else 0)
            + (d_token if corr_use_Hp else 0)
        )
        if corr_mlp:
            # MLP 头：已训 backbone 的残差通常不是其自身特征的线性可读函数，给非线性容量
            self.proj = nn.Sequential(
                nn.Linear(corr_in_dim, d_token),
                nn.GELU(),
                nn.Linear(d_token, horizon),
            )
        else:
            self.proj = nn.Linear(corr_in_dim, horizon)
    ...
    def forward(
        ...
        with torch.autocast(device_type=H.device.type, enabled=False):
            ...
            qcc_out = Hp
            if self.corr_use_Hp:
                Hp_norm = self.hp_ln(Hp)  # 在残差加和被 H 淹没前保存独立通道
            if self.use_layer_norm:
                qcc_out = self.ln(H_in + qcc_out)  # 残差 + LN
            if S is not None and (self.corr_use_S or self.corr_gate_S):
                S_norm = self.s_ln(S)  # (B, V, 2M)
            # 变体 1：对齐后频谱直接拼接进修正（特征换位置）
            if S is not None and self.corr_use_S:
                qcc_out = torch.cat([qcc_out, S_norm], dim=-1)  # (B, V, d+2M)
            # 变体 4：Hp 独立通道（消息传递输出不被残差结构淹没）
            if self.corr_use_Hp:
                qcc_out = torch.cat([qcc_out, Hp_norm], dim=-1)  # (B, V, 2d)
            # 变体 3：主预测拼进修正（误差反馈）
            if self.corr_use_ymain:
                qcc_out = torch.cat([qcc_out, y_main.transpose(1, 2)], dim=-1)  # (B, V, d+H)
            # 预测修正：(B, V, d[+2M][+H]) → (B, V, H) → (B, H, V)
            correction_raw = self.proj(qcc_out).transpose(1, 2)
            # 变体 2：频谱门控（逐变量 σ(Linear(S)) ⊙ corr）
            if S is not None and self.corr_gate_S:
                gate = torch.sigmoid(self.s_gate(S_norm))  # (B, V, 1)
                correction_raw = correction_raw * gate.transpose(1, 2)  # (B, 1, V)
