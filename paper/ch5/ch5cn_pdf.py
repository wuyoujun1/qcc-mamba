...
E.append(Paragraph("5.5　可解释性", h2s))
...
E.append(Paragraph("表 6　耦合核 f 与数据耦合的斯皮尔曼相关。ρ 为斯皮尔曼秩相关系数（见正文），取 f 的非对角元与滞后对齐互相关统计量的成对排序；合并行把所有 ETT 变量对拼接后计算，p 值来自该合并样本。", cap))
E += mk_table(["数据集（或合并）","斯皮尔曼 ρ"],
 [["ETTh1","0.07"],["ETTm2","0.40"],["ETTh2","-0.05"],["ETTm1","0.14"],
  ["ETTh1+ETTm2+ETTh2+ETTm1 合并","0.25"]], [7*cm,3*cm])
E.append(Paragraph("为考察耦合项的实际作用，我们在推理阶段按平均耦合强度屏蔽一部分变量对（不重新训练）：分别屏蔽最强的一半与最强的四分之三，测量测试误差的相对变化。表 7 在多个数据集与预测长度上给出结果：屏蔽最强的一半已使误差上升，当屏蔽比例提高到四分之三时误差进一步显著增大，说明耦合整体上被模型所依赖，且其作用分散在多对变量之间，而非集中在个别连接上。", body))
E.append(Paragraph("表 7　按比例屏蔽变量对耦合后的测试误差变化（推理阶段执行，不重新训练）。数值为相对完整模型的测试误差变化百分比；“最强”指按平均耦合强度排序后最强的那部分变量对。", cap))
