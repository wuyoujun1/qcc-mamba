...
## 4. 现在做到哪了
- 主表/消融/可解释性（含**表 7 按比例屏蔽耦合**）已写进 `paper/ch5/第五章初稿_中文_20260909.pdf`（= 生成器 `ch5cn_pdf.py` 的当前输出，**第五章定版**；改完重跑即可）。
...
2. 在新配置上重跑 **消融**（去掉量子核/耦合应显著变差）与 **表 7 的屏蔽实验**，确认结论一致。
...
```
> ⚠️ `ch5_supp_20260909/` 下的脚本默认 `os.chdir` 到工作副本 `/home/youjun/dataops_ws/`（读那里的 `run.py` / `logs/` / `checkpoints/`）。换机或不在此路径时，用 `QCC_WS=<工作副本路径>` 覆盖，例如 `QCC_WS=/tmp/ws python paper/ch5/ch5_supp_20260909/eval_coupling.py <model_id> 0`。
