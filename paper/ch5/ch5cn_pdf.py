...
E.append(Paragraph("5.1　实验设置", h2s))
...
E.append(Paragraph("我们以 S-Mamba[1] 为主干，在其编码器层之间插入耦合模块，得到 QCCK-M。基线复现。每一条基线都使用其公开发布的实现与论文推荐的配置。官方 S-Mamba 按官方脚本在本机复现，在核心数据集上 96 步与本机复现核对一致，偏差不超过 2.5%，表中的 S-Mamba 即本机官方复现值。iTransformer、DLinear、PatchTST、TimesNet、SAMBA、CMamba、Affirm 与 DeMa 在统一的长时序评测框架下，以各自的公开实现与默认配置复现，全部方法在同一划分、同一输入长度与同一指标口径下比较。逐条而言，iTransformer[3] 把自注意力作用在变量维，DLinear[4] 以分解加线性取胜，PatchTST[5] 采用分块与通道独立，TimesNet[6] 挖掘多周期结构，CMamba 与 SAMBA 属于 Mamba[2] 状态空间家族，Affirm 与 DeMa[7] 为近期的 Transformer 预测方法，S-Mamba 即我们所增强的主干基线。实现细节与训练协议。所有模型用 PyTorch 实现，实验在 8 张 NVIDIA RTX 4090 GPU 上完成。对每一条基线，我们采用其公开发布的模型结构与官方推荐的超参数；若训练出现显存不足，就把批量减半。对我们提出的 QCCK-M，我们在验证集上对量子比特数与学习率做小范围网格搜索，以确定一组默认配置。训练按验证损失早停；对两个最大的数据集，主干编码维度按官方 S-Mamba 对齐到 512，避免把增益误归于模型规模。我们提出的 QCCK-M 在时域 MSE 之上加入频域监督项[8]，该项权重取 1.0。量子核采用固定配置：量子比特数按验证集确定为 5；所有耦合分析均基于原始保真度核 f。主结果为单次运行，消融实验对两个随机种子取平均。主结果将 QCCK-M 与官方 S-Mamba 基线以及八条近期基线对比，这些基线覆盖主要技术路线，包括 Mamba 与状态空间族的 CMamba 与 SAMBA，Transformer 族的 iTransformer、PatchTST、Affirm 与 DeMa，多尺度卷积模型 TimesNet，以及线性模型 DLinear。", body))
...
E.append(Paragraph("5.2　主结果", h2s))
...
E.append(Paragraph("与八条经典基线相比，QCCK-M 在 ETTh2、ETTm2、ECL、Traffic 与 Beijing 这些核心多变量数据集的全部格点上优于八条基线，在其余数据集上保持竞争力，通常与最强基线的差距在千分之几以内。表中的每个格子都同时给出 MSE 与 MAE，两数来自同一次运行；Beijing 上八条基线的结果仍在数据工作文件中补全。", body))
...
E.append(Paragraph("表 3　ETTh1 与 ETTm2 在 96 步上的跨变量输入灵敏度，即非对角与对角响应之比。", cap))
...
E.append(Paragraph("5.4　消融", h2s))
E.append(Paragraph("表 5 在五个耦合数据集上分离量子核与频域监督两组件的贡献。消融围绕量子核与频域监督两个设计展开，并以同特征维数的 RBF 核作为量子核的经典替代方案。Full 表示量子核加频域监督的完整模型；A1 去掉量子核而保留频域监督，用来检验量子核相对主干加频域监督的增量；A2 把量子核换成同特征维数的 RBF 核而保留频域监督，用来检验量子核相对经典 RBF 核的增量；A3 保留量子核而去掉频域监督，用来检验频域监督的作用；A4 在 A2 的基础上去掉频域监督，作为去掉两者的下界参照。表 4 汇总各变体的组件构成，表 5 给出数值结果。Full 相对 A2 在多数数据集与预测长度上保持优势，说明性能提升并非仅来自核化操作本身；频域监督的收益随步长增大。", body))
E.append(Paragraph("表 4　各消融变体的组件构成。Full 为 QCCK-M 完整模型；A1 去除量子核，A2 以 RBF 核替代量子核，A3 去除频域监督，A4 同时以 RBF 替代量子核并去除频域监督。", cap))
E += mk_table(["组件","量子核","RBF 核","频域监督"],
 [["Full","有","无","有"],["A1","无","无","有"],
  ["A2","无","有","有"],["A3","有","无","无"],
  ["A4","无","有","无"]],
 [4.2*cm,2.8*cm,2.8*cm,2.8*cm])
E.append(Paragraph("表 5　消融数值结果，每格给出 MSE 与 MAE；其中 Full 一列的数值与主表里 QCCK-M 对应格完全一致，保证两张表自洽。", cap))
...
E.append(Paragraph("5.5　敏感性", h2s))
E.append(Paragraph("量子核的可调设置主要是量子比特数与可学习门初值。图 2 按数据集分列（ETTh1 与 ETTm2）、按超参数分行，给出两者相对各自默认取值的 MSE 变化：量子比特数取 3、5、7，门初值取 0.0、0.1、1.0，每条折线对应一个预测长度。可以看到，改变任一超参数后 MSE 的变化幅度都很小（多数在 ±1% 以内，个别设置到约 2%），且方向并不一致：例如 ETTm2 在 192 步上取 3 个量子比特、或门初值取 0，都略优于默认，而在其 96 步上反而略差。这说明量子核性能对这两个超参数不敏感，也不存在随取值单调变化的“最优点”；为避免逐设置调参，全部实验固定采用量子比特数 5、门初值 0.1 这一组中间取值，而不再声称该取值最优。表 6 给出对应数值。", body))
E.append(Paragraph("表 6　量子比特数与门初值在各设置上的 MSE。默认列对应量子比特数 5、门初值 0.1；n_qubits=3/7 两列把量子比特数改为 3/7 而门取默认 0.1，gate=0.0/1.0 两列把门初值改为 0.0/1.0 而量子比特数取默认 5。", cap))
sens2 = [["数据集-H","默认","n_qubits=3","n_qubits=7","gate=0.0","gate=1.0"],
         ["ETTh1 96","0.3749","0.3756","0.3746","0.3749","0.3747"],
         ["ETTh1 192","0.4263","0.4268","0.4274","0.4265","0.4268"],
         ["ETTh1 336","0.4686","0.4688","0.4681","0.4684","0.4685"],
         ["ETTh1 720","0.5506","0.5494","0.5526","0.5504","0.5505"],
...
E += mk_table(["数据集（或合并）","斯皮尔曼 ρ"],
 [["ETTh1","0.07"],["ETTm2","0.40"],["ETTh2","-0.05"],["ETTm1","0.14"],
  ["ETTh1+ETTm2+ETTh2+ETTm1 合并","0.25"]], [7*cm,3*cm])
E.append(Paragraph("为考察耦合项的实际作用，我们在推理阶段按平均耦合强度屏蔽一部分变量对（不重新训练）：分别屏蔽最强的一半与最强的四分之三，测量测试误差的相对变化。表 8 在多个数据集与预测长度上给出结果：屏蔽最强的一半已使误差上升，当屏蔽比例提高到四分之三时误差进一步显著增大，说明耦合整体上被模型所依赖，且其作用分散在多对变量之间，而非集中在个别连接上。", body))
E.append(Paragraph("表 8　按比例屏蔽变量对耦合后的测试误差变化（推理阶段执行，不重新训练）。数值为相对完整模型的测试误差变化百分比；“最强”指按平均耦合强度排序后最强的那部分变量对。", cap))
